...
export async function generateTaxConfirmPdf(importId) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // 表格内容 - 更紧凑的行高
      const rowHeight = 11
      const pageBottom = 750  // 页面底部边界，留空间给汇总信息
      ...
      const drawTableHeader = () => {
        ...
        // 表头
        doc.fontSize(8)
        let headerX = tableLeft + 2
        headers.forEach((header, i) => {
          doc.text(header, headerX, y, { width: colWidths[i], align: 'left' })
          headerX += colWidths[i]
        })
      ...
      items.forEach((item, index) => {
        ...
        if (y > pageBottom) {
          ...
          // 新页面的页眉
          doc.fontSize(10)
             .text('税费确认单 - 货物明细 (续)', pageLeft, y, { align: 'center', width: pageWidth })
          doc.fontSize(8)
             .text(`批次号: ${batch.importNo}`, pageLeft, y + 15)
          y += 30
      ...
      doc.fontSize(7)
         .fillColor('#666666')
         .text(`共 ${items.length} 件商品`, tableLeft, y)
      doc.fillColor('#000000')
      y += 10
      
      // 检查是否需要换页来显示汇总信息（汇总+确认+签名约需要120像素）
      if (y > 680) {
        doc.addPage()
        y = 40
      }
