...
| **演示环境** | PostgreSQL | Render 云端 PostgreSQL |
| **生产环境** | PostgreSQL | Render 云端 PostgreSQL |
...
# 或者连接到 Render 测试数据库（需要 External URL）
# DATABASE_URL_TEST=postgresql://用户:密码@主机/数据库名?sslmode=require
```
...
Render 云端连接示例：
```
postgresql://user:password@oregon-postgres.render.com/logistics_db?sslmode=require
```
...
Render PostgreSQL 提供自动每日备份功能，可在 Dashboard 中管理。
